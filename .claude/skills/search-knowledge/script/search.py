...
async def search_knowledge(query: str, max_results: int = 5) -> Dict[str, Any]:
    ...
    if not query:
        return {"answer": "查询内容不能为空。", "total_found": 0, "query": ""}

    # 医疗查询可能包含敏感信息，日志只记录长度，不记录原文。
    logger.info("Searching knowledge base: query_length={}, max_results={}", len(query), max_results)

    # 获取知识库单例（避免重复加载模型）
    kb = await asyncio.to_thread(get_knowledge_base)

    # 使用 Milvus 进行语义检索
    # Milvus 与嵌入模型均为同步接口，不应阻塞 Agent 的事件循环。
    ...
    for doc in results:
        formatted_results.append({
            ...
            "content": safe_knowledge_excerpt(doc["content"], doc["metadata"]),
            "source": doc["metadata"].get("source", "医学知识库"),
            "score": doc["score"],
            "type": doc["metadata"].get("type"),
            "provenance_notice": provenance_notice(doc["metadata"]),
        })
    ...
    if formatted_results:
        return {
            ...
            "query": query,
            "sources": [item["source"] for item in formatted_results],
        }
    else:
        return {
...
def format_results(results: list) -> str:
    ...
    for i, doc in enumerate(results, 1):
        ...
        output.append(f"来源: {doc.get('source', '未知')}")
        output.append(doc.get("provenance_notice", ""))
