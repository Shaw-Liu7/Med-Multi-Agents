...
async def search_similar_cases(query: str, max_results: int = 3) -> Dict[str, Any]:
    ...
    logger.info(
        "Similar-case Skill invoked through unscoped fallback: query_length={}",
        len(query),
    )
    # 外部长期检索会发送医疗查询，必须由 AgentLoop 注入同一 LongTermMemory、
    # 可信 tenant/user/session 以及逐请求 consent；不能在 Skill 内自行构造客户端。
    return {
        "answer": "相似历史只能在已授权且带可信用户作用域的 AgentLoop 中检索。",
        "total_found": 0,
        "query": query,
        "success": False,
