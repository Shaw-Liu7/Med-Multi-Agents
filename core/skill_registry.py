...
class SkillRegistry:
    ...
    async def execute(self, name: str, **kwargs) -> Dict[str, Any]:
        ...
        Returns:
            Skill 执行结果
        """
        if name in {"search_history", "search_similar_cases"}:
            return {
                "success": False,
        ...
        try:
            # Tool arguments may contain symptoms or identifiers; never log them.
            ...
            logger.error(
                f"Skill execution failed: skill={name}, error_type={type(exc).__name__}"
            )
            return {
                "success": False,
