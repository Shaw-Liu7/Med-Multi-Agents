        ...
        if content.startswith('---'):
            ...
            if end_idx != -1:
                ...
                try:
                    ...
                    logger.warning(
                        f"Skill metadata parse failed: error_type={type(exc).__name__}"
                    )
        return None
    except Exception as exc:
        logger.warning(
            f"Skill metadata read failed: error_type={type(exc).__name__}"
        )
        return None
...
def discover_skills(project_root: Path = None) -> List[Dict]:
    ...
    for skill_dir in skills_dir.iterdir():
        ...
        try:
            ...
            logger.warning(
                f"Skipping skill {skill_name}: error_type={type(exc).__name__}"
            )
            continue
