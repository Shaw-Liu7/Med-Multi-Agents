...
class KnowledgeBase:
    ...
    def __init__(
        ...
        if allow_model_download is None:
            allow_model_download = os.getenv(
                "MEDIX_ALLOW_MODEL_DOWNLOAD", "false"
            ).lower() in {"1", "true", "yes"}
        ...
        # 内存模式使用无三方依赖的稳定哈希向量，用于离线测试/降级。
        # 真实 Qdrant 后端才加载 SentenceTransformer。
        if not use_memory and EMBEDDING_AVAILABLE:
            try:
                ...
                logger.warning(
                    "Embedding model unavailable; using deterministic memory fallback: {}",
                    type(e).__name__,
                )
        else:
            if use_memory:
                self.vector_size = 256
                logger.info("Using in-memory storage with deterministic hash embeddings")
        ...
        if not use_memory and QDRANT_AVAILABLE and self.embedding_model:
            try:
                ...
                logger.warning(
                    "Qdrant unavailable; using in-memory storage: {}",
                    type(e).__name__,
                )
                self.qdrant_client = None
        else:
    ...
    def _init_collection(self):
        ...
        try:
            ...
            logger.error("Failed to initialize collection: {}", type(e).__name__)
            self.qdrant_client = None
    ...
    def _embed_text(self, text: str) -> Optional[List[float]]:
        ...
        try:
            ...
            logger.error("Failed to embed text: {}", type(e).__name__)
            return None

    def _fallback_embed(self, text: str) -> List[float]:
        """为无模型的内存后端生成稳定、归一化的哈希向量。

        它不替代语义 embedding，但比原先“内存模式无法添加文档”
        更适合离线测试和故障降级。
        """
        size = self.vector_size or 256
        vector = [0.0] * size
        normalized = " ".join((text or "").lower().split())
        tokens = list(normalized) + [normalized[i : i + 2] for i in range(max(0, len(normalized) - 1))]
        for token in tokens:
    ...
    async def add_document(
        ...
        # 向量化
        vector = await asyncio.to_thread(self._embed_text, content)
        if vector is None:
        ...
        if self.qdrant_client:
            ...
            try:
                ...
                logger.error("Failed to add document to Qdrant: {}", type(e).__name__)
                return ""
        else:
            # 存储到内存
    ...
    async def search(
        ...
        score_threshold = max(-1.0, min(float(score_threshold), 1.0))
        ...
        if self.qdrant_client:
            ...
            try:
                search_result = await asyncio.to_thread(
                    ...
                    limit=top_k * 5 if filters else top_k,
                ...
                logger.error("Qdrant search error: {}", type(e).__name__)
                return []
        else:
            # 从内存检索（简单的余弦相似度）
            results = []
            for doc in self.memory_storage.values():
                if doc.vector and self._matches_filters(doc.metadata, filters):
                    # 计算余弦相似度
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self.qdrant_client:
            try:
                ...
                logger.error("Failed to get Qdrant stats: {}", type(e).__name__)
                return {"backend": "qdrant", "error": "stats_unavailable"}
        else:
            return {
                "backend": "memory",
...
async def search_knowledge_base(
    ...
    Returns:
        相似文档列表
    """
    # 默认禁止模型下载；只有本地已有模型和已配置 Qdrant 时才使用外部后端。
    kb = KnowledgeBase()
